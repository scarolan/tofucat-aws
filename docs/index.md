name: AWS-OpenTofu-Workshop
class: center
count: false
![:scale 60%](images/tf_aws.png)
<br><br>
# AWS OpenTofu Workshop
...
Welcome to the beginner's guide to OpenTofu on AWS. This slide deck is written entirely in Markdown language, which means you can make edits or additions, then submit a pull request to add your changes to the main branch. To make edits to the slide deck simply fork this repository, edit the Markdown files, and submit a pull request with your changes.
...
1. Intro to OpenTofu & Demo
1. OpenTofu Basics<br>
👩‍🔬 **Lab - Setup and Basic Usage**
1. OpenTofu In Action: plan, apply, destroy
1. Organizing Your OpenTofu Code<br>
🧪 **Lab - OpenTofu in Action**<br>
1. Provision and Configure AWS Instances<br>
🔬 **Lab - Provisioning with OpenTofu**<br>
1. Manage and Change Infrastructure State<br>
1. OpenTofu Cloud<br>
⚗️ **Lab - OpenTofu Remote State**
...
---
name: Provision-with-OpenTofu-2
# Method 3: Provision with OpenTofu
```hcl
...
```
.center[Example OpenTofu code for building an AWS instance.]

???
**And finally we have option #3, OpenTofu. OpenTofu uses a Domain Specific Language, or DSL that is designed to be both human-friendly and machine-readable. This is an example snippet of OpenTofu code. Now watch as I flip back to the previous slide. Would you rather have to write and maintain this complex and messy JSON, or this simple, compact OpenTofu code?**

Advance back to the previous slide to illustrate the difference between JSON and equivalent OpenTofu.
...
???
**So what exactly _is_ OpenTofu? OpenTofu is the DNA of your hybrid infrastructure.**
...
**It sounds funny but the truth is your wiki is the obituary of the last known state of your infrastructure. One of the main reasons to use OpenTofu is because it is self-documenting. The code itself explains every step required to build this infrastructure, and therefore it is always up to date.**
...
* Provide a codified workflow to create infrastructure
* Change and update existing infrastructure
* Safely test changes using **`tofu plan`** in dry run mode
???
...
**What if instead we had a way to safely test every change that went into production with a dry run? What would actually happen if I ran this code right now? OpenTofu comes with a built in dry run mode that allows you to visualize exactly what would happen if you pushed the apply button right now. This is a valuable tool for sysadmins and operations teams who prize stability and uptime.**

**Unexpected changes in the tofu plan output can be investigated _before_ they go into production.**
...
???
**As you expand your OpenTofu usage, you'll have certain patterns and pieces of your infrastructure that you'd like to re-use. Maybe you want your network security to be set up a certain way, every time. Or perhaps someone wrote a great OpenTofu config for your web application. OpenTofu supports custom modules, which are simply packages of pre-built OpenTofu code that others can use. You can use OpenTofu modules to avoid repetition, enforce security, and ensure that standards are followed.**
...
OpenTofu can be used across *all* major cloud providers and VM hypervisors.

???
**Every major cloud provider has their own JSON or YAML based provisioning tool. But all of them are written in YAML or JSON. And if you learn one of these systems, guess what, the others are completely different. Now if you want to have a multi-cloud strategy you have to learn three separate provisioning systems. With OpenTofu you are using the same language, the same simple syntax on all three of these cloud providers.**
...
OpenTofu code (HCL) is easy to learn and easy to read. It is also 50-70% more compact than an equivalent JSON configuration.
...
???
1Password did a great blog post illustrating the difference between AWS Cloudformation (JSON) and OpenTofu.

https://blog.1password.com/terraforming-1password/

1Password were able to move their entire production infrastructure to OpenTofu in a few short weeks. Now they can tear down and completely rebuild their production environment in a matter of hours.

---
Name: Why-OpenTofu-on-AWS
# Why OpenTofu on AWS?

* Supports multi-cloud & hybrid infrastructure

???
**Why specifcially should you use OpenTofu on AWS? The first reason is that OpenTofu supports your hybrid or multi-cloud strategy. If you need to build some infrastructure on-prem, and some in AWS, OpenTofu is a natural fit. Your technical staff only has to learn a single language to be able to provision in either environment.**
...
???
**OpenTofu is also great for migrating between cloud providers. Let's say you wanted to move some workloads from AWS to AWS. The code changes in OpenTofu would be much easier to implement than they would via CloudFormation Templates. I was able to migrate a simple demo application from one cloud to another in a few short hours, because there was almost no learning curve. OpenTofu code looks the same no matter where you run it.**
...
???
**It's not unusual to see provisioning times drop from days or weeks to hours or minutes when users adopt OpenTofu. Ineffective manual steps and change approvals can be replaced with fast code pipelines that have rigorous testing and security built right in. Now instead of waiting for days for a change request to be approved, users can self-provision their infrastructure without bottlenecks or slow approval processes.**
...
???
**Every modern IT organization has to deal with risk. It's a balancing act between security and usability. You can make it so secure nobody can use it, or on the other end you have a free for all where users can do whatever they want, but are putting the entire cloud account in jeopardy due to risky behavior. OpenTofu allows you to reduce risk by abstracting your users away from the web UI or API. Instead we provide a safe, auditable abstraction layer that lets users get their work done in a secure and safe way, that doesn't grant unnecessary privileged access.**
...
**This is a workstation just like the ones you'll be using for today's workshops. I'm going to run a tofu apply command to build out the lab environment. We're actually cheating a little bit here, as we prebaked most of the environment before class to save us some time. Just like your favorite cooking show!**

**You can see the results of the OpenTofu run here in my shell. This output is showing me the URL of the application server I just built. And if we pop over here to the AWS portal you'll see all of the different parts of my lab environment.**

**This is Infrastructure as code. By the end of today's training you'll be able to create your own infrastructure using OpenTofu.**
...
???
**Now that you have OpenTofu installed and working with AWS, we can do a few dry runs before building real infrastructure. Follow along carefully, copying and pasting the commands on each slide into your shell as we go.**
...
It ships as a single binary which is written in Go. OpenTofu is cross platform and can run on Linux, Windows, or MacOS.

Installing OpenTofu is easy. You simply download a zip file, unzip it, and run it.

---
name: opentofu-command-line
class: col-2
# OpenTofu Command Line
OpenTofu is a command line tool.

OpenTofu commands are either typed in manually or run automatically from a script.

The commands are the same whether you are on Linux or Windows or MacOS.

OpenTofu has subcommands that perform different actions.
...
```tex
*$ tofu help
Usage: tofu [-version] [-help] <command> [args]
...
Common commands:
...
```
Type `tofu subcommand help` to view help on a particular subcommand.
???
**This is a good command to know. OpenTofu help will reveal all the available subcommands.**
...
An OpenTofu workspace is simply a folder or directory that contains OpenTofu code.

OpenTofu files always end in either a `*.tf` or `*.tfvars` extension.

Most OpenTofu workspaces contain a minimum of three files:

**main.tf** - Most of your functional code will go here.<br>
**variables.tf** - This file is for storing variables.<br>
**outputs.tf** - Define what is shown at the end of a OpenTofu run.<br>
...
```
OpenTofu fetches any required providers and modules and stores them in the .terraform directory. If you add, change or update your modules or providers you will need to run init again.

???
**OpenTofu has an extendible architecture. You download the core program, tofu, then it fetches plugins and modules that are required for your code.**
...
```
Preview your changes with `tofu plan` before you apply them.

???
**`tofu plan` is a dry run command. We're not actually building anything yet, OpenTofu is just telling is what it would do if we ran it for real.**

---
name: defining-variables
# Where are Variables Defined?
OpenTofu variables are placed in a file called *variables.tf*. Variables can have default settings. If you omit the default, the user will be prompted to enter a value. Here we are *declaring* the variables that we intend to use.
...
---
name: lab-exercise-1
# 👩‍💻 Lab Exercise: OpenTofu Basics
<br><br>
In this lab you'll learn how to set up your editor, use the OpenTofu command line tool, integrate with AWS, and do a few dry runs with different settings.
...
.contents[
In this chapter we:
* Used the **`tofu init`** command
* Ran the **`tofu plan`** command
...
Every OpenTofu resource is structured exactly the same way.
...
**name** = Arbitrary name to refer to this resource. Used internally by OpenTofu. This field *cannot* be a variable.
...
The OpenTofu core program requires at least one provider to build anything.

You can manually configure which version(s) of a provider you would like to use. If you leave this option out, OpenTofu will default to the latest available version of the provider.
...
???
**This specifies the earliest version that the module is compatible with while leaving the user of the module flexibility to upgrade to newer versions of OpenTofu without altering the module.**
...
```
`tofu apply` runs a plan and then if you approve, it applies the changes.
...
```
`tofu destroy` does the opposite. If you approve, your infrastructure is destroyed.
???
**OpenTofu can just as easily destroy infrastructure as create it. With great power comes great responsibility!**

---
name: tofu-fmt
# OpenTofu Format
OpenTofu comes with a built in code formatter/cleaner. It can make all your margins and list indentation neat and tidy. Beauty works better.
...
class: compact
# OpenTofu Dependency Mapping
OpenTofu can automatically keep track of dependencies for you. Look at the two resources below. Note the highlighted lines in the aws_instance resource. This is how we tell one resource to refer to another in OpenTofu.
...
# Organize Your OpenTofu Code
OpenTofu will read any file in your workspace that ends in a `.tf` extension, but the convention is to have a main.tf, variables.tf, and outputs.tf. You may add more tf files if you wish.
...
The first file is called main.tf. This is where you normally store your OpenTofu code. With larger, more complex infrastructure you might break this up across several files.
...
class: compact
# The Outputs File
The outputs file is where you configure any messages or data you want to show at the end of a tofu apply.

```hcl
output "catapp_url" {
  value = "http://${aws_route53_record.tofucat.fqdn}"
}


output "private_key" {
  value = "${tls_private_key.tofucat.private_key_pem}"
}
```
...
class: img-right
# OpenTofu Dependency Graph
.center[![:scale 100%](images/blast_radius_graph_1.png)]

The OpenTofu resource graph visually depicts dependencies between resources.
...
# 👩‍💻 Lab Exercise: OpenTofu in Action
Let's use OpenTofu to build, manage, and destroy AWS resources. In this lab exercise you'll build the tofucat application stack by running the `tofu apply` command.

🛑 **STOP** after you complete the third quiz.

???
**We will explore the OpenTofu Graph together once everyone has completed the lab. Once you have the graph running in your instruqt lab stop there.**
...
In this chapter we:
* Learned about OpenTofu resources
* Ran tofu plan, graph, apply and destroy
...
# Using OpenTofu Provisioners
Once you've used OpenTofu to stand up a virtual machine or container, you may wish to configure your operating system and applications. This is where provisioners come in. OpenTofu supports several different types of provisioners including: Bash, Powershell, Chef, Puppet, Ansible, and more.

.center[https://www.terraform.io/docs/provisioners/index.html]

???
**OpenTofu works hand-in-hand with these other configuration management tools to install packages, configure applications and change OS settings inside of a virtual machine or container.**

---
name: file-provisioner
class: compact
# The File Provisioner
The OpenTofu file provisioner copies files onto the remote machine.
...
class: compact
# OpenTofu & Config Management Tools
.center[![:scale 50%](images/cpa.jpg)]

OpenTofu works well with common config management tools like Chef, Puppet or Ansible.

Run Puppet with 'local-exec':<br>
https://www.terraform.io/docs/provisioners/local-exec.html

OpenTofu and Ansible - Better Together:<br>
https://github.com/scarolan/ansible-terraform

---
name: provisioner-tips
# OpenTofu Provisioner Tips
OpenTofu provisioners like remote-exec are great when you need to run a few simple commands or scripts. For more complex configuration management you'll want a tool like Chef or Ansible.

Provisioners only run the first time a OpenTofu run is executed. In this sense, they are not idempotent. If you need ongoing state management of VMs or servers that are long-lived, we recommend using a config management tool.
...
.contents[
In this chapter we:
* Learned about OpenTofu Provisioners
...
class: compact
# OpenTofu State
OpenTofu is a _stateful_ application. This means that it keeps track of everything you build inside of a **state file**. You may have noticed the terraform.tfstate and terraform.tfstate.backup files that appeared inside your working directory. The state file is OpenTofu's source of record for everything it knows about.
...
Sometimes infrastructure may be changed outside of OpenTofu's control.
...
Whenever you run a plan or apply, OpenTofu reconciles three different data sources:

1.  What you wrote in your code
2.  The state file
3.  What actually exists

OpenTofu does its best to add, delete, change, or replace existing resources based on what is in your *.tf files. Here are the four different things that can happen to each resource during a plan/apply:
